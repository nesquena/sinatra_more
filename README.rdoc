...
This component provides Sinatra with an enhanced url routing system which enables named routes to be defined
and used throughout your application. The benefits of this are that instead of having to hard-code route urls
into every link and definition, now we can define the urls in a single spot and then attach an alias
which can be used to refer to the url in the rest of the application. 
...
The routing system also supports url route namespaces:
...
You can then reference the url using the same <tt>url_for</tt> method:

  <%= link_to 'admin page', url_for(:admin, :show, :id => 25) %>
  
You can freely use both named routes and traditional Sinatra routes in the same application without any conflicts.
