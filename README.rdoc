...
powered in part by the excellent Thor gem (incidentally also used in the Rails 3 generator). These generators are intended
to allow easy code generation both for creating new applications and adding to existing ones. The generators have been
built to be as agnostic as possible, supporting a myriad of test frameworks, js libraries, mocking libraries and more. 

The usage for the generator is very simple:
...
This would construct a Sinatra application DemoApp inside the folder 'demo_app' which extends from Sinatra::Application. 
Inside the application there would be configuration and setup performed for the default components. You can also
modify specific components to be used:

  $ sinatra_gen demo_app . --test=rspec --renderer=haml --mock=rr --script=jquery --orm datamapper
  
The available components and their default options are listed below:

* test:     <tt>bacon</tt> (default), <tt>shoulda</tt>, <tt>rspec</tt>
* renderer: <tt>haml</tt> (default), <tt>erb</tt>
* mock:     <tt>rr</tt> (default), <tt>mocha</tt>
...
The generator framework within sinatra_more is highly extensible and additional generators can easily be added. This would be done 
through forking our project and reading through the code in <tt>/generators/sinatra_generator.rb</tt> and
the setup instructions inside each file within <tt>/generators/components/</tt>. We are happy to accept pull requests
for additional component types not initially included (although helping us maintain them would also be appreciated).

We are also planning to add additional generator actions such as:
